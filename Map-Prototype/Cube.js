class Cube{
  ...
  render(M) {
    ...
    // front
    drawTriangle3DUV( [-s*b,-s,-s*b,  s*t,s,-s*t,  s*b,-s,-s*b], [0,0, 1,1, 1,0] );
    drawTriangle3DUV( [-s*b,-s,-s*b,  -s*t,s,-s*t,  s*t,s,-s*t], [0,0, 0,1, 1,1] );
    ...
    drawTriangle3DUV( [-s*t,s,-s*t,  -s*t,s,s*t,  s*t,s,s*t], [0,0, 0,1, 1,1] );
    drawTriangle3DUV( [-s*t,s,-s*t,  s*t,s,s*t,  s*t,s,-s*t], [0,0, 1,1, 1,0] );

    // bottom
    //gl.uniform4f(u_FragColor, rgba[0]*.9, rgba[1]*.9, rgba[2]*.9, rgba[3]);
    drawTriangle3DUV( [-s*b,-s,-s*b, -s*b,-s,s*b, s*b,-s,s*b], [0,0, 0,1, 1,1] );
    drawTriangle3DUV( [-s*b,-s,-s*b, s*b,-s,s*b, s*b,-s,-s*b], [0,0, 1,1, 1,0] );

    //right side
    gl.uniform4f(u_FragColor, rgba[0]*.95, rgba[1]*.95, rgba[2]*.95, rgba[3]);
    drawTriangle3DUV( [s*b,-s,-s*b, s*t,s,-s*t, s*t,s,s*t], [1,0, 1,1, 0,1] );
    drawTriangle3DUV( [s*b,-s,-s*b, s*t,s,s*t, s*b,-s,s*b], [1,0, 0,1, 0,0]);
